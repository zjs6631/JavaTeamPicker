...
public class A9Smith {
    ...
    public void clearGameBoard() {
        ...
        for (int row = 0; row < gameBoard.length; row++) {
            for (int column = 0; column < gameBoard[row].length; column++) {
                gameBoard[row][column] = TicTacToe.cellValues.empty; //set each cell to the enum value empty
            }
        }
        return; //return
    ...
    public void setCellValue(int row, int col) {
        ...
        if (gameBoard[row][col] != TicTacToe.cellValues.empty) {
    ...
    public String toString() {
        ...
        for (int row = 0; row < gameBoard.length; row++){ //for loop with another nested for loop to visit each cell
            for (int col = 0; col < gameBoard[row].length; col++){
                if (gameBoard[row][col].equals(TicTacToe.cellValues.empty)){ //if the cell holds the enum value empty
